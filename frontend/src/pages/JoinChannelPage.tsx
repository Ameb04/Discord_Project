import { Globe, Hash, LoaderCircle, Lock, LogIn, Radio, Users } from "lucide-react";
import { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";

import { getChannelInvite, joinChannelViaInvite } from "../api/channels";
...
import type { ChannelInvitePreview } from "../types/chat";

/**
 * The landing page for a channel invite link.
 *
 * Joining is a POST rather than a side effect of the GET: opening a link — from
 * a preview crawler, a prefetch, or a stray refresh — should show what the
 * channel is, not silently enrol the visitor. The button is the consent, which
 * is also why an invite works for a private channel where self-joining does not.
 */
function JoinChannelPage() {
  const { token } = useParams<{ token: string }>();
  const navigate = useNavigate();

  const [invite, setInvite] = useState<ChannelInvitePreview | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadInvite() {
      ...
      try {
        const preview = await getChannelInvite(token);
        ...
        setError(
          apiStatus(err) === 404
            ? "This invite link has been revoked, or the channel no longer exists."
  ...
  async function handleJoin() {
    ...
    try {
      const channel = await joinChannelViaInvite(token);
      navigate(`/channels/${channel.id}`, { replace: true });
    } catch (err) {
      setJoinError(
        apiErrorMessage(err, "Could not join this channel.", ["token"])
      );
    } finally {
      setIsJoining(false);
    }
          ...
          <>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
              ...
              <Badge variant="secondary" className="gap-1">
                ...
                {invite.topic_count}{" "}
                {invite.topic_count === 1 ? "topic" : "topics"}
              </Badge>
              <Badge variant="secondary" className="gap-1">
                {invite.access_level === "public" ? (
                  <>
            ...
            {invite.bio.trim() ? (
              <p className="mt-4 text-sm leading-7 break-words text-muted-foreground">
                {invite.bio}
              </p>
            ) : null}
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              {invite.is_member ? (
                <Button asChild>
                  <Link to={`/channels/${invite.id}`}>Open channel</Link>
                </Button>
              ) : (
                <Button
                  type="button"
