...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        // `leading-6` overrides the tight line-height that ships with
        // `text-sm`: it clips the descenders of Persian and Arabic glyphs,
        // which sit lower than anything in the Latin range.
        "flex h-11 w-full min-w-0 rounded-xl border border-input bg-white/[0.04] px-4 py-1 text-sm leading-6 text-foreground shadow-sm transition-[color,box-shadow] outline-none",
